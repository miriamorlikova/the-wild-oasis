		...
		<nav className="border-r border-primary-900">
			<ul className="flex flex-col gap-2 h-full text-lg">
				{navLinks.map((link) => (
					<li key={link.name}>
						<Link
							className={`py-3 px-5 hover:bg-primary-900 hover:text-primary-100 transition-colors duration-400 flex items-center gap-4 font-semibold text-primary-200 rounded-sm`}
							href={link.href}
						>
							{link.icon}
							<span>{link.name}</span>
